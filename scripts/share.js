 ...
 *
 * Ctrl+C stops the server and takes the link offline.
 */
...
// ── 1b. Cloudflare quick tunnel (the fallback) ───────────────────────────────
...
function ensureBuild() {
  ...
  if (r.status !== 0) die("Build failed. Fix the errors above, then run `npm run share` again.");
}
...
function announce(url, fixed) {
  ...
  line("  Press Ctrl+C here to take it offline.");
  line("══════════════════════════════════════════════\n");
}
...
(async () => {
  ...
  if (!(await waitForServer(() => serverDied))) {
    server.kill();
    die(`Server never came up on port ${PORT}. Is MySQL running? Is that port already in use?`);
  }
  ...
  let tunnel;
  if (fixedUrl) {
    // Clear anything an earlier run left behind (e.g. the window was closed
    // instead of Ctrl+C), so we start from a clean funnel config.
  ...
  const shutdown = () => {
    ...
    // Belt and braces: make sure the fixed link really stops answering.
    if (fixedUrl) spawnSync(ts, ["funnel", "reset"], { stdio: "ignore" });
